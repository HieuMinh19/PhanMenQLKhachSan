...
public class frmBooking_Step2 extends JInternalFrame {
	...
	ArrayList<JEditorPane> dataRows = new ArrayList<JEditorPane>();
...
//		page = 0;
		...
		for(int i = 0; i < pageSize; i++) {
			...
			edDV01.setText(data.get(i * page).getTenDichVu());
			String  strGiaDV =  String.valueOf(data.get(i * page).getGiaDichVu());
			edGiaDV01.setText(strGiaDV);
			...
			btnAddDV01.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent arg0) {
					...
					int iSL = Integer.parseInt( edSLDV01.getText() );
					int iMaCTDatPhong = ctdp.getMaCTDatPhong();
					int iDonGia = data.get(j * page).getGiaDichVu();
					int iThanhTien = iSL * iDonGia;
					int iMaDV = data.get(j * page).getMaDichVu();
...
//			int m = data.size() % pageSize;
			...
			button.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent arg0) {
					...
					pages.forEach(p -> getContentPane().remove(p));
