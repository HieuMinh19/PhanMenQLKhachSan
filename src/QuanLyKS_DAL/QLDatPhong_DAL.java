...
public class QLDatPhong_DAL extends JInternalFrame {
	public QLDatPhong_DAL() {
...
//	}
	...
	public ArrayList<CTDatPhong_DTO> LoadListNV (){
		...
		String query = "SELECT NgayThucHien,NgayNhan,NgayTra , kh.MaKhachHang as 'MaKhachHang',HoTen,lp.TenLoaiPhong,lp.MaLoaiPhong as 'MaLoaiPhong' FROM CTDATPHONG ctdp, KHACHHANG kh, LOAIPHONG lp, PHONG p WHERE ctdp.MaKhachHang=kh.MaKhachHang and  p.MaLoaiPhong = lp.MaLoaiPhong and ctdp.MaPhong = p.MaPhong"  ;
		try {
			ptmt = conn.prepareStatement(query);
			ResultSet rs = ptmt.executeQuery();
			ArrayList<CTDatPhong_DTO> dsnv = new ArrayList<CTDatPhong_DTO>();
			while(rs.next()) {
				CTDatPhong_DTO nv = new CTDatPhong_DTO();
				//nv.setMaNhanVien(rs.getInt("MaNhanVien"));
			//	nv.setHoTen(rs.getString("HoTen"));
				//Date NgayNhan = ;
//				java.util.Date a = rs.getDate("NgayThucHien");
//				nv.setNgayThucHien(a);
//				nv.setCMND(rs.getInt("CMND"));
//				nv.setNgayVaoLam(rs.getDate("NgayVaoLam"));
//				//nv.setMaChucVu(rs.getInt("MaChucVu"));
//				nv.setLoaiPhong(new LoaiPhong_DTO(rs.getInt("MaLoaiPhong"),rs.getString("TenLoaiPhong")));
//				dsnv.add(nv);
			}
			return dsnv;
		} catch (SQLException e) {
